...
import Matcher from './marcheur-matcher';
import { JSDOM } from 'jsdom';
import xpath from 'xpath';
import { oprx } from './omml2mathml-operators';
const MATH_NS = 'http://www.w3.org/1998/Math/MathML';
const nsMap = {
  m: 'http://schemas.openxmlformats.org/officeDocument/2006/math'
};
let select = xpath.useNamespaces(nsMap);
let selectAttr = (path, attr, ctx, onlyDef = false) => {
...
// `omml2mathml` is a small utility to convert from Microsoft's OMML to MathML.
// The input of `omml2mathml` should be an `m:oMath` or `m:oMathPara` element. Note that you can always use the former
...
// It returns an instance of a `math` element as an HTML DOM object.

// AUTHORS NOTE: This is by and large a port of the the `omml2mathml.xsl` XSLT that Microsoft ships with Office and
...
export default function omml2mathml(omml) {
  ...
  return walker
    ...
    .match(m.el('m:f'), (src, out, w) => {
      let type = (
          selectAttr('./m:fPr[last()]/m:type', 'm:val', src) || ''
        ).toLowerCase(),
        outer =
          type === 'lin'
            ? el('mrow', {}, out)
    ...
    .match(m.el('m:m'), (src, out, w) => {
      let mcjc = selectAttr(
        ...
        outer = el(
          'mtable',
          mcjc && mcjc !== 'center' ? { columnalign: mcjc } : {},
          out
        );
    ...
    .match(m.el('m:rad'), (src, out, w) => {
      let degHide =
        selectAttr('m:radPr[last()]/m:degHide', 'm:val', src) || false;
    ...
    .match(m.el('m:nary'), (src, out, w) => {
      let subHide =
        selectAttr('m:naryPr[last()]/m:subHide', 'm:val', src) || false;
      if (subHide) subHide = forceFalse(subHide);
      let supHide =
        selectAttr('m:naryPr[last()]/m:supHide', 'm:val', src) || false;
      if (supHide) supHide = forceFalse(supHide);
      let limLocSubSup = selectAttr(
        'm:naryPr[last()]/m:limLoc',
        'm:val',
        src
      ).toLowerCase();
    ...
    .match(m.el('m:acc'), (src, out, w) => {
      let mover = el('mover', { accent: 'true' }, out),
        row = el('mrow', {}, mover),
        acc =
          selectAttr('m:accPr/m:chr', 'm:val', src).substr(0, 1) || '\u0302',
    ...
    .match(m.el('m:borderBox'), (src, out, w) => {
      let hideTop = forceTrue(
          selectAttr(
            'm:borderBoxPr[last()]/m:hideTop[last()]',
            'm:val',
            src
          ).toLowerCase()
        ),
        hideBot = forceTrue(
          selectAttr(
            'm:borderBoxPr[last()]/m:hideBot[last()]',
            'm:val',
            src
          ).toLowerCase()
        ),
        ...
        strikeH = forceTrue(
          selectAttr(
            'm:borderBoxPr[last()]/m:strikeH[last()]',
            'm:val',
            src
          ).toLowerCase()
        ),
        strikeV = forceTrue(
          selectAttr(
            'm:borderBoxPr[last()]/m:strikeV[last()]',
            'm:val',
            src
          ).toLowerCase()
        ),
...
function parseMT(ctx, out, { toParse = '', scr, sty, nor }) {
  ...
  if (!startsWithOper && !startsWithNum) {
    ...
    let mi = el(
      'mi',
      tokenAttributes({ scr, sty, nor, charToPrint, tokenType: 'mi' }),
      out
    );
    ...
    let num = numStart(toParse),
      mn = el(
        'mn',
        tokenAttributes({ scr, sty: 'p', nor, tokenType: 'mn' }),
        out
      );
    mn.textContent = num;
  ...
  if (toParse[0] === '&') {
    ...
    if (!startsWithOper && !startsWithNum) {
      if (!nor) {
        let mi = el(
          'mi',
          tokenAttributes({ scr, sty, nor, charToPrint: 1, tokenType: 'mi' }),
          out
        );
      ...
      if (!nor) {
        let mo = el(
          'mo',
          tokenAttributes({ nor, charToPrint: 1, tokenType: 'mo' }),
          out
        );
        mo.textContent = toParse.substr(0, 1);
      } else {
      ...
      if (!nor) {
        let mn = el(
          'mn',
          tokenAttributes({ sty: 'p', nor, charToPrint: 1, tokenType: 'mn' }),
          out
        );
...
function createEqArrRow(w, src, out, align, cur) {
  ...
  if (select('following-sibling::*', cur).length) {
    ...
    createEqArrRow(
      ...
      out,
      (align + (amp % 2)) % 2,
      select('following-sibling::*', cur)[0]
    );
  }
}
...
function setup(src, out, w) {
  ...
  const nod = nodal(doc, {}, nsMap);
  const math = doc.createElementNS(MATH_NS, 'math');
